...
function msgMenu({ registered, focusLine }) {
  return (
    ...
    (focusLine ? `\n${focusLine}\n` : "\n") +
    `\nChọn nhanh:\n` +
    `📅 Hôm nay — lịch / đang học trong ngày\n` +
    `📌 Buổi tới — buổi kế tiếp (chưa bắt đầu)\n` +
...
/**
 * Nút "Buổi tới" — CHỈ hiện buổi chưa bắt đầu (day = upcoming).
 * focus dùng để ghi chú "đang học buổi khác".
 */
function msgNext(day, focus = null) {
  if (!day) {
    let note = "";
    if (focus?.current && (focus.status === "ongoing" || focus.status === "ending_soon")) {
      note =
        `\n\nℹ️ Hiện bạn <b>đang học</b> buổi ${esc(focus.current.date)} ` +
        `(${esc(focus.current.time || "")}).\n` +
        `Không còn buổi nào sau đó trong lịch.`;
    }
    return (
      `${header("📌", "Buổi tới")}\n` +
      `Không còn buổi học sắp tới trong lịch đã xếp.` +
      note +
      `\n\nChờ cập nhật Chủ nhật hoặc xem web.`
    );
  }

  let banner = `${header("📌", "Buổi tới")}\n`;
  // Đang học buổi khác → nhắc ngắn, phần chính là buổi kế
  if (focus?.current && (focus.status === "ongoing" || focus.status === "ending_soon")) {
    const left =
      focus.minsLeft != null ? ` · còn ~${focus.minsLeft}p` : "";
    banner +=
      `ℹ️ <i>Đang học hôm nay${left} — bên dưới là <b>buổi kế tiếp</b></i>\n` +
      `${divider()}\n`;
  } else if (focus?.status === "upcoming" && focus.minsUntil != null) {
    if (focus.minsUntil < 60) {
      banner += `⏳ Còn khoảng <b>${focus.minsUntil} phút</b> nữa bắt đầu.\n${divider()}\n`;
    } else if (focus.minsUntil < 24 * 60) {
      const h = Math.floor(focus.minsUntil / 60);
      banner += `⏳ Còn khoảng <b>${h} giờ</b> nữa bắt đầu.\n${divider()}\n`;
    }
  }

  return banner + fmtDayCard(day);
}
