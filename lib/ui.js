...
function inlineMain(extra = {}) {
  const rows = [
    [
      ...
      { text: "📌 Buổi kế tiếp", callback_data: "nav:next" },
    ],
    [
...
function replyMenu() {
  return {
    keyboard: [
      [{ text: "📅 Hôm nay" }, { text: "📌 Buổi kế tiếp" }],
...
function replyMenuStopped() {
  return {
    keyboard: [
      ...
      [{ text: "📌 Buổi kế tiếp" }, { text: "🗓 Tuần này" }],
...
const BOT_COMMANDS = [
  ...
  { command: "next", description: "Buổi kế tiếp (chưa bắt đầu)" },
