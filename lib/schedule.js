...
function nextClass(data, now = new Date()) {
  ...
  for (const day of data.days || []) {
    if (!day.date_iso) continue;
    const { h, m } = parseStartHm(day.time);
    // so sánh theo phút từ epoch ảo theo lịch VN
    const startMin =
      Date.parse(`${day.date_iso}T00:00:00+07:00`) / 60000 + h * 60 + m;
    const nowMin =
      Date.parse(`${vn.iso}T00:00:00+07:00`) / 60000 + vn.hour * 60 + vn.minute;
    // còn hiệu lực nếu chưa qua ~3h sau giờ bắt đầu
    if (startMin + 180 >= nowMin) {
      candidates.push({ startMin, day });
    }
  }
