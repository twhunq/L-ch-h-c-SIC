...
/**
 * Buổi cần quan tâm:
 * - đang học → buổi hiện tại
 * - chưa tới giờ → buổi sắp tới
 * - hết hết → null
 */
...
/** Chỉ buổi chưa bắt đầu (bỏ buổi đang học). */
function upcomingClass(data, now = new Date()) {
  const f = classFocus(data, now);
  if (f.status === "upcoming") return f.next;
  return f.next; // nếu đang học → buổi sau; nếu none → null
}
